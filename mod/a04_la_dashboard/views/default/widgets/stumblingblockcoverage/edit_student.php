...
$clipit_user = elgg_extract('clipit_user', $vars);
$activity_ids = ClipitUser::get_activities($clipit_user->id, true);
...
if ( $vars['entity']->scale == ClipitGroup::SUBTYPE) {
  $group_options = LADashboardHelper::getGroupBundlePHP($vars['entity']->activity_id,false,$clipit_user);
    ...
    $('#<?php echo "scale_dropdown-$widget_id" ?>').change(function () {
        ...
        if (document.getElementById('<?php echo "scale_dropdown-$widget_id" ?>').selectedOptions[0].value == "<?php echo ClipitGroup::SUBTYPE?>") {
            ...
            elgg.get('ajax/view/metrics/get_groups', {
                data: {
                    id: activityId,
                    clipit_user:<?php echo $clipit_user->id ?>,
                    add_all:"false"
                },
