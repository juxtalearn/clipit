...
class phpSesame
{
	...
	public function listRepositories()
	{
		...
		{
			throw new Exception ('phpSesame engine response error');
		}
	...
	public function contexts($resultFormat = self::SPARQL_XML)
	{
		...
		$request = new HTTP_Request2($this->dsn . '/repositories/' . $this->repository . '/contexts', HTTP_Request2::METHOD_GET);
		$request->setHeader('Accept: ' . self::SPARQL_XML);
	...
	public function size($context = 'null')
	{
		...
		$request = new HTTP_Request2($this->dsn . '/repositories/' . $this->repository . '/size?context=' . $context, HTTP_Request2::METHOD_GET);
		$request->setHeader('Accept: text/plain');
