...
class phpSesame
{
    ...
    public function update($query, $resultFormat = self::SPARQL_XML, $queryLang = 'sparql')
    {
        $this->checkRepository();
        $this->checkQueryLang($queryLang);
        $this->checkResultFormat($resultFormat);

        $request = new HTTP_Request2($this->dsn . '/repositories/' . $this->repository . '/statements', HTTP_Request2::METHOD_POST);
        $request->setHeader('Accept: ' . self::SPARQL_XML);
        $request->addPostParameter('content-type', 'application/x-www-form-urlencoded');
        $request->addPostParameter('update', $query);
       // echo $query;
        echo "\n";
       // echo $this->dsn . '/repositories/' . $this->repository . '/statements';
        echo "\n";
        echo "\n";
        echo "\n";
