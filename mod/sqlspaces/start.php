<?php
	...
	function log_to_sqlspace($object, $event) {
		...
		//Leaving of groups is not loggable by the elgg core, that's why we use this code to augment the system log:
		if ($object != null && $event == 'leave') {
			if (!empty($object) && count($object) == 2) {
				if (array_key_exists('group', $object) && array_key_exists('user', $object)) {
					$group = $object['group'];
					$user = $object['user'];
					...
					if (array_key_exists('guid', $user) && array_key_exists('guid', $group)) {
						$performed_by = $user->guid;
						$object_content = $group->guid;
						...
						mysqli_query($con,"INSERT INTO `sqlslogging` ".
									 ...
									 "VALUES (".$object_id.", '".$transaction_id."', 'ElggRelationship', 'relationship', 'member', 'delete', '".$time."', '".$ip_address."', '".$performed_by."', 2, 'yes', 0, '".$object_content."');");
					}
				}
			}
		}
		if ($object instanceof Loggable) {
			...
			if(isset($data[0])) {
				...
				if ($entity->showga) {
						...
						//Wenn es eine Annotation ist, wollen wir den content...
						if ($object_class == "ElggAnnotation") {
							$object_content = $object->value;
						}
						//Wenn es eine Relationship ist, wollen wir die zweite ID...
						else if ($object_class == "ElggRelationship"){
							$object_content = $object->guid_two;
						}
						//Ansonsten nehmen wir die Desc
						else {
						...
						mysqli_query($con,"INSERT INTO `sqlslogging` ".
											"(object_id, transaction_id, object_class, object_type, object_subtype, event, time, ip_address, performed_by, access_id, enabled, owner_guid, content) ".
									 "VALUES (".$object_id.", '".$transaction_id."', '".$object_class."', '".$object_type."', '".$object_subtype."', '".$event."', '".$time."', '".$ip_address."', '".$performed_by."', ".$access_id.", '".$enabled."', ".$owner_guid.", '".$object_content."');");
				}
			}
		}
	}
