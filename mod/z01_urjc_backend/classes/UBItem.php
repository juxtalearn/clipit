...
class UBItem {
     ...
     * @param ElggObject $elgg_object Object to load instance from, if passed
     *
     * @throws APIException
     */
            ...
            if(empty($elgg_object)){
                if(!$elgg_object = new ElggObject($id)) {
                    throw new APIException(
                        "ERROR: Failed to load " . get_called_class() ." object with ID '" . $id . "'.");
                }
            }
     ...
     *
     * @param array $prop_array Properties to return (with key = ID). Default = [all] (object instance)
     ...
     */
    static function get_all($limit = 0, $prop_array = null) {
        $object_array = array();
        ...
        if(!empty($elgg_entity_array)) {
            foreach($elgg_entity_array as $elgg_entity) {
                if(empty($prop_array)){
                    $object_array[(int)$elgg_entity->guid] = new static((int)$elgg_entity->guid);
                }
                foreach($prop_array as $property) {
                    switch($property) {
                        case "id":
                            $object_array[] = (int)$elgg_entity->guid;
                            break;
                        case "login":
                            $object_array[(int)$elgg_entity->guid] = $elgg_entity->username;
                            break;
                        default:
                            $object_array[(int)$elgg_entity->guid] = $elgg_entity->$property;
                            break;
                    }
                }
            }
