...
*               $oFile->title = "Logo";
        ...
        if( (isset($file['name']) && !empty($file['name'])) && ($input_name=='logo_img' || $input_name == 'bg_img') ){
            ...
            if(file_get_simple_type($mime_type) == "image"){
                $folder = elgg_get_plugins_path()."z03_clipit_theme/graphics/icons/";
                ...
                elgg_set_plugin_setting($input_name, $file_name.".".$file_ext, "z03_clipit_theme");
            }
        }
    }
}
foreach ($params as $k => $v) {
    if (!elgg_set_plugin_setting($k, $v, 'z03_clipit_theme')) {
        register_error(sprintf(elgg_echo('plugins:settings:save:fail'), 'z03_clipit_theme'));
        //forward(REFERER);
    }
}
//forward(REFERER);
