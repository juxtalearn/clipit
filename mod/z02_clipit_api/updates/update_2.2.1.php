...
foreach($activity_array as $activity_id){
    $elgg_object = get_entity($activity_id);
    $tricky_topic = $elgg_object->get("tricky_topic");
    if(!empty($tricky_topic)){
        ClipitActivity::set_properties($activity_id, array("tricky_topic" => (int)$tricky_topic));
    }
...
foreach($example_array as $example_id){
    $elgg_object = get_entity($example_id);
    $tricky_topic = $elgg_object->get("tricky_topic");
    if(!empty($tricky_topic)){
        ClipitExample::set_properties($example_id, array("tricky_topic" => (int)$tricky_topic));
    }
    $elgg_object->set("tricky_topic", null);
}
...
foreach($quiz_array as $quiz_id){
    $elgg_object = get_entity($quiz_id);
    $tricky_topic = $elgg_object->get("tricky_topic");
    if(!empty($tricky_topic)){
        ClipitQuiz::set_properties($quiz_id, array("tricky_topic" => (int)$tricky_topic));
    }
    $elgg_object->set("tricky_topic", null);
}